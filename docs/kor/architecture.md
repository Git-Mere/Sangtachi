...
**과대 데이터그램에서 비우기를 멈추지 않는다.** `MAX_DATAGRAM`(1472)을 넘는 데이터그램도 반환되는 시점에는 이미 소켓 큐에서 사라진 상태다. 거기서 `return EMPTY` 하면 그 바퀴의 비우기가 한 개에서 끝나므로, 과대 데이터그램을 섞어 보내는 것만으로 배칭이 무력화된다. 버리고 계속 비운다. 다른 오류는 원인을 모르므로 그 바퀴를 끝낸다.

**수신 버퍼는 `MAX_DATAGRAM + 1`(1473)바이트다.** 버퍼를 `MAX_DATAGRAM` 크기로 잡으면 과대 패킷을 `WSAEMSGSIZE`라는 **오류 코드로만** 알 수 있고, 그 판정이 Winsock의 절단 동작에 의존한다. 1바이트 크게 잡으면 `n > MAX_DATAGRAM`이라는 **길이 비교**가 판정이 되고, 시험에서 1473바이트를 보내 이 경로를 결정적으로 재현할 수 있다. **그래도 `WSAEMSGSIZE` 분기를 지우지 않는다.** 1473바이트를 넘는 데이터그램은 여전히 그 오류로 올라온다. 두 자리는 같은 사건이므로 카운터도 같은 것을 쓴다.
...
**주입 실패는 폐기한다.** `DATA`가 [`protocol.md`](protocol.md) 8.4 검증을 통과하면 `WintunAllocateSendPacket`으로 송신 링의 자리를 잡고 `WintunSendPacket`으로 넣는다. 송신 링에 자리가 없어 할당이 실패하는 것은 고장이 아니라 **부하가 높을 때 정상적으로 일어나는 일이다.** 그 패킷을 버리고 `drop_inject_error`를 올린다. **재시도하지 않는다.** 이 스레드가 타이머와 두 수신 경로를 같이 책임지므로, 자리가 날 때까지 기다리면 그동안 keepalive와 `HELLO` 재전송이 멈춘다. 링이 찰 만큼 소비가 밀린 상태에서는 그 자리에서 다시 시도해도 같은 곳에서 또 막힌다. 버리는 대가는 내부 패킷 한 개다. **그 대가가 회복되는지는 내부 프로토콜에 달렸다.** 내부가 TCP면 재전송이 메우므로 지연이고(4장의 Minecraft 경로가 그렇다), UDP 같은 비신뢰 프로토콜이면 그 패킷은 그대로 잃는다. 터널은 내부를 구분하지 않으므로(NFR-2) 이것을 "항상 지연일 뿐"이라고 적지 않는다.

**지금 확정하는 것은 카운터 하나와 동작 계약 넷이다.** 카운터는 `drop_inject_error` 하나다. 동작은 **주입이 실패하면 그 패킷을 버리고, 카운터를 올리고, 재시도하지 않고, 루프를 멈추지 않는다.** 이 넷은 어떤 API 모양에서도 성립한다.
...
**이 절의 카운터는 [`protocol.md`](protocol.md)의 카운터와 계층이 다르다.** `drop_oversize_datagram`과 `drop_inject_error`는 소켓과 어댑터 API가 낸 결과를 세는 것이라 이 문서가 정의한다. protocol.md 7장과 8장의 `drop_*`는 와이어 분류와 검증 파이프라인의 카운터다. 목록을 만들 때 두 출처를 섞지 않는다.
...
- `[loop]`는 고정 크기 SPSC 링 버퍼에 레코드를 넣는다. 생산자 하나, 소비자 하나뿐이라 원자 head/tail만으로 **락 없이** 구현된다. 가득 차면 **새 레코드를 버리고** `telemetry_queue_dropped` 를 올린다. 대기 없음, 실패 보고 없음.
- **링은 256 항목이다.** 근거는 **자동 주기 지표의 배경 생산 속도**다. `PING` 5초, keepalive 15초처럼 주기가 정해진 것만 보면 평균 1건/초 미만이고, 그 속도에서 256칸은 분 단위 분량이라 업로드 하나가 소켓 타임아웃(3.2.7)에 걸려 소비가 몇 초 밀리는 구간을 덮는다. **버스트는 이 크기가 보장하지 않는다.** 시험이 유발하는 연속 연결 시도, 수동 RTT 표본, 폐기 카운터가 몰리는 구간은 같은 초에 여러 건을 만들 수 있고 그러면 더 빨리 찬다. 그때는 새 레코드가 버려진다. **그것이 의도한 동작이다.** 지표는 유실을 허용하는 물건이고, 버스트를 덮으려고 크기를 올리면 장애 구간에서 오래된 지표를 더 오래 붙들 뿐이다.
- **콘솔 명령 큐는 16 줄이고 같은 정책을 쓴다.** 가득 차면 새 줄을 버리고 `console_queue_dropped` 를 올린다. **두 큐의 카운터를 같은 이름으로 쓰지 않는다.** 이름이 없으면 Phase 1 검증이 무엇을 보는지 정할 수 없다. 생산자가 `[console]`, 소비자가 `[loop]` 하나씩이라 여기도 락이 필요 없다. 사람이 치는 명령이 16줄이나 밀리는 상황은 `[loop]`가 이미 막혔다는 뜻이고, 그때 큐를 키워도 처리되지 않는다.
...
`session_epoch`는 **세션 시도 단위**이고 프로세스 단위가 아니다. 이 필드가 없으면 재시작이나 재시도 후 이전 시도의 지연 패킷이 새 세션에 수용된다. 수신 측은 첫 유효 패킷에서 상대 epoch를 고정하고, 이후 다른 epoch의 `HELLO`는 상대가 새 시도를 시작했다는 뜻이므로 폐기가 아니라 재협상으로 처리하고, 나머지 타입은 폐기한다. 추첨 방법, 폐기 목록, 재협상 절차는 [`protocol.md`](protocol.md) 4.3과 9.3, 9.5가 갖는다.
...
- 펀치 마감에 걸린 두 전이는 같은 마감을 쓰고 **그 시점에 선 플래그 개수로만** 코드가 갈린다. 판정 근거는 8장에 있다.

**그림에 없는 되돌아가는 전이가 하나 있다.** 상대가 새 시도를 시작하면(다른 `session_epoch`의 `HELLO`) 세션은 `CONNECTED`에서도 왕복 증거를 지우고 `CONNECTED` 이전으로 돌아간다. 절차와 그때의 상태 판정은 [`protocol.md`](protocol.md) 9.5가 갖는다. 이 문서만 읽고 전이가 단방향이라고 구현하면 그 경로가 빠진다.

위 그림은 정상 경로의 뼈대다. **활성 상태 셋**(`PUNCHING`, `HANDSHAKING`, `CONNECTED`)과 모든 수신 패킷의 전이는 [`protocol.md`](protocol.md) 9.4절 전이표에 있고, `IDLE`과 종료 상태(`FAILED`, `CLOSED`)의 수신 처리는 9.4.1절이 따로 맡는다. 특히 `PUNCHING`에서 `HELLO_ACK`이 먼저 도착하는 것과 `CONNECTED`에서 `HELLO`에 다시 응답하는 것은 정상 동작이며, 이를 빠뜨리면 양쪽이 타임아웃한다.
...
**뒤의 두 코드는 패킷 종류가 아니라 플래그 성립 개수로 가른다.** 두 플래그의 뜻과 어느 쪽이 먼저 서는지 정해져 있지 않다는 점은 5.3에 있고, 판정 규칙의 출처는 [`protocol.md`](protocol.md) 9.6이다. "상대 `HELLO`를 받았는가"로 가르면 `HELLO_ACK`이 먼저 도착하는 정상 경로에서 `got_ack`만 선 채 마감을 맞은 세션이 `HOLE_PUNCH_TIMEOUT`으로 기록된다. 그 세션은 우리 패킷이 상대에게 닿았음을 이미 증명했으므로 홀펀칭은 실패하지 않았다. 코드가 틀리면 Phase 9의 실패 단계 분포가 그만큼 틀린다.
...
| 항목 | 계약 |
|------|------|
| 내용 | 연결 시도 한 건마다 결과(성공 또는 [`spec.md`](spec.md) FR-13 의 실패 코드)와 RTT. **RTT 는 측정하지 못한 시도에서는 비운다** |
| 시점 | 두 번이다. **수립이 끝나는 시점**에 한 줄(성공이든 실패든), 그리고 **세션이 끝나는 시점**에 한 줄을 더한다 |
...
[`protocol.md`](protocol.md) 5.5 에 따라 조회에 실패해 `drop_unmatched_pong` 으로 버려진다.
**즉 `CONNECTED` 이전에는 RTT 표본이 만들어지지 않는다.** 9.4 가 그 구간에서 `PING` 에
응답하고 `PONG` 을 매칭 절차에 넣는 것과 모순되지 않는다. 그래서 그 전에
끝난 시도(`STUN_DISCOVERY_FAILED`, `CONTROL_PLANE_EXCHANGE_FAILED`, `HOLE_PUNCH_TIMEOUT`,
`PEER_HANDSHAKE_FAILED`)에는 값이 없다. **없는 것을 0 으로 적지 않는다.** 0ms 는 측정된 값처럼
보이고 Phase 9 의 집계를 오염시킨다. 빈 값으로 적고, 빈 값의 표기는 줄 형식과 같이 정한다.
...
**RTT 도 같은 이유로 두 줄이 필요하다.** `PING` 송신은 `CONNECTED` 진입에 시작하므로(11장) **수립이 끝나는 순간에는 RTT 표본이 아직 없다.** 첫 줄의 RTT 는 그래서 대개 비어 있다. 두 번째 줄이 세션이 끝날 때 쓰이므로 거기에 값이 들어간다. 수립 줄을 첫 표본까지 미루지 않는 이유는 그 사이에 프로세스가 죽으면 기록이 아예 남지 않기 때문이다.

- **수립 줄**: 모든 시도에 쓴다. 결과 코드(성공 또는 실패). RTT 는 그 시점에 표본이 있으면 적고 없으면 비운다
- **종료 줄**: **`CONNECTED` 에 도달한 시도에만 쓴다.** 종료 코드(`CLOSED` 또는 `TUNNEL_DROPPED`)와 마지막 RTT 표본, 없으면 빈 값
...
Phase 2~3의 검증 항목이 "`STUN_DISCOVERY_FAILED` 기록", "로그로 확인"을 판정 수단으로 쓴다([`roadmap.md`](roadmap.md)). 무엇을 어디에 어떤 모양으로 내는지가 없으면 그 검증을 통과했는지 판정할 수 없다. **그 항목들이 말하는 "기록"은 이 로그다.** 로컬 기록 파일은 Phase 4의 산출물이라 Phase 2~3에는 아직 없다. 최소 계약은 넷이다.

| 항목 | 계약 |
|------|------|
| 출력처 | 표준 오류(stderr). 버퍼링에 갇히지 않아 크래시 직전 줄이 남고, 표준 출력과 따로 리다이렉트할 수 있다 |
| 수준 | `INFO`, `WARN`, `ERROR` 셋. 더 나누지 않는다 |
| 단위 | 한 줄에 이벤트 하나. **줄바꿈과 제어문자는 공백 하나로 바꿔 싣는다.** 이스케이프 표기를 쓰지 않는다. 한 줄 한 이벤트라는 성질만 지키면 되고, 표기를 늘리면 읽는 쪽이 그것을 해석해야 한다 |
...
| `session.state` | `from`, `to` ([`protocol.md`](protocol.md) 9.1의 상태 이름 문자열 그대로) | Phase 4 전이 확인 |
...
**로그는 위의 로컬 기록 파일과 다른 물건이다.** 로그는 사람이 읽는 진단이라 줄이 늘거나 문구가 바뀌어도 되고, 기록 파일은 [`spec.md`](spec.md) M-6 판정의 입력이라 위의 계약을 지킨다. **M-6 판정을 로그에서 읽지 않고, 진단을 기록 파일에 늘리지 않는다.** 둘을 섞으면 진단 문구를 고칠 때마다 M-6 판정이 같이 흔들린다. **Phase 1~3 검증이 위의 고정 이벤트 키를 읽는 것은 다른 이야기다.** 그 Phase 에는 기록 파일이 아직 없고, 그래서 키와 필드 이름을 고정해 둔 것이다.
...
현재 소스 트리는 루트의 `src/main.cpp`와 `CMakeLists.txt`뿐이다 (`docs/`는 이미 채워져 있다). `client/` 하위로 옮기고 `control-server/`를 만드는 재배치가 남아 있다.
