...
**리뷰 지적을 기각하면 그 결정을 문서 안에 적는다.** 커밋 기록에만 적으면 다음 리뷰어는 그것을
보지 않으므로 같은 지적이 라운드마다 다시 올라온다. 근거는
`docs/kor/commit_history/2026-09-21-followup4-spec-logic.md` 의 2라운드다.
...
있다. Phase 4 착수 전에 그 문서에서 정한다. 코드에서 임의로 정하지 않는다.

**지금 `docs/eng` 는 `docs/kor` 보다 뒤처져 있다.** 한국어를 먼저 고치고 푸시 지시가 있을 때
미러를 만드는 방식으로 작업했다. 그래서 `docgate.py` 는 현재 `mirror` 와 `parity` 가 걸린다.
미러를 맞춘 직후에 게이트로 돌린다.
