...
- A `HELLO_ACK` echoing the nonce we sent was received. That is evidence for the self -> peer path
- A `HELLO_ACK` was sent in reply to a valid `HELLO` from the peer. That is evidence for the peer -> self path

Evidence from one side alone, or our own successful transmission alone, is not a verdict. The
decision formula is set by `protocol.md` 9.2 `CONNECTED` Condition.
...
isolation mechanism is chosen. The boundary contract is in `architecture.md` 3.4 Telemetry
Service.
...
| C-5 | If scope is exceeded, **remove from the lowest priority first.** The tiers and their cost are set by "Cost of priority removal" below. |

#### Cost of priority removal

C-5 has five tiers.

| Tier | What it holds | Phase |
|------|---------|-------|
...
This records which success criteria disappear when a tier is dropped.

> **Why.** Without this, "remove from the lowest first" becomes an instruction that breaks the
> minimum deliverable.
...
| # | Criterion |
|---|------|
| M-1 | The client runs on two Windows machines on different networks |
| M-2 | Both machines discover a usable public endpoint with their own STUN client |
| M-3 | The two peers exchange connection information through the AWS control plane |
| M-4 | A direct UDP connection is established in a supported NAT environment without manual port forwarding |
| M-5 | Inner IP packets are transferred over the direct tunnel |
| M-6 | Connection state and basic performance data are recorded |

How each criterion is judged is right below. Putting the criterion and the verdict on one line
takes the table cell past 400 characters.

#### How M is judged

**M-1.** There are two verdicts.

- Normal startup. Each machine's log shows the actual local endpoint that was bound, and the
  process does not exit afterwards. That value is the contract in
  [`protocol.md`](protocol.md) chapter 6 Socket Ownership (bind to `INADDR_ANY` and port 0,
  read the actual port with `getsockname`)
- Different networks. The public IPs in the M-2 logs differ between the two machines. If the
  public IP is the same, this criterion does not recognize them as different networks

Even with the same public IP we do not assert "behind the same NAT". Different networks behind a
shared CGNAT can appear with the same public IP.

> **Why.** The startup verdict does not use the STUN result because M-1 would then become a
> subset of M-2. Then "it started but STUN does not work" would be visible in no criterion.

**M-2.** Each machine's log prints the local endpoint and the public endpoint. The public
endpoint is confirmed by **querying two different STUN servers from the same UDP socket** and
observing the same IP:Port. That is the same verdict as the supported condition above.

- HTTPS-based IP check services are recorded for reference only. They are a TCP path that may
  differ from the UDP transmission path
- That the discovered IP:Port is actually reachable is confirmed in M-4 and M-5

**M-3.** Both machines receive, in the `get_peers` ready response, the peer's virtual IP and
**the public endpoint printed in the peer machine's M-2 log**.

- The response format is set by [`control_plane.md`](control_plane.md) 4.5 `get_peers`
- It is read from `virtual_ip` and `candidates` of the `control.peers` event
  ([`architecture.md`](architecture.md) chapter 9) in the client log. `candidates` must contain
  the peer's `stun.result` `mapped` value
- The verdict is made from the logs of both machines separately. Only one side receiving it is
  not an exchange

**M-4.** The session reaches `CONNECTED` with no router port forwarding rule. Packet capture
confirms the path does not go through AWS.

**M-5.** A `DATA` round trip succeeds in both directions and received bytes match sent bytes.

- The payload is a **complete inner IPv4 packet** as defined by `protocol.md` 5.4 `DATA`
...
Control plane upload and per-environment aggregation are FR-14 and Phase 9.
...
| # | Criterion |
|---|------|
| A-1 | Repeated experiments are performed across multiple network environments |
| A-2 | Successful and failed NAT traversal cases are compared |
| A-3 | Experiment results are visualized and limitations are documented |
| A-4 | A repeatable experiment procedure exists |

How each criterion is judged is right below.

#### How A is judged

**A-1.** Measurement results are produced for each of: same LAN, home network to home network,
home network to mobile hotspot, campus network to home network.

**A-2.** Each failure keeps its per-stage code and NAT environment information, aggregated by
observed mapping behavior. NAT type is not asserted. **Both controlled failure scenarios** are
run.

| Condition applied | Code that must be recorded |
|-----------|----------------------|
| UDP is blocked with the firewall | `STUN_DISCOVERY_FAILED` |
| An unresponsive endpoint is injected | `HOLE_PUNCH_TIMEOUT` |

Write down what each is evidence of, and keep them apart. A controlled failure is evidence that
the failure handling path works. It is not evidence about the failure rate or failure
distribution in real environments. The failure rate comes only from the repeated measurements of
A-1. If there are zero real failures, write zero. Do not substitute controlled failures.

**A-3.** [`experiments.md`](experiments.md) contains graphs and a statement of limitations.

**A-4.** The pass condition is that **one person who is not the author** performs one
reproduction to completion from the document alone. The reproducer proceeds without asking for
explanation outside the document and records where they got stuck. Do not declare "reproducible"
without a trial.

> **Why.** The author finds missing premises in their own document only with difficulty by
> reading alone.
