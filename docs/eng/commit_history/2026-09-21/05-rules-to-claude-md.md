...
every round. The evidence is round 2 of `2026-09-21/04-followup4-spec-logic.md`.
...
| 1 | The evidence for the new rule is a follow-up 4 commit record, so a stale reference remains | **Rejected.** It is a commit record, not an audit record, and the evidence for the rule exists only there. The existing `CLAUDE.md` cites `2026-09-21/01-docgate.md` the same way. **The rejection reason was written inside `CLAUDE.md`** |
