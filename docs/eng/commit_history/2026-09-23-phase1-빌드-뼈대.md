...
| Build | `hamychi_core` (static) + `hamychi_client` (executable) + `hamychi_tests`. `main()` does not go into the library |
| Warnings | `/W4 /WX /permissive- /utf-8` on the `hamychi_warnings` interface target. It is linked only into targets of this repo |
| Tests | Catch2 v3.16.0 pinned by tag through `FetchContent`. Registered into CTest with `catch_discover_tests` |
| Code | `hamychi::network::WsaContext`. The constructor calls `WSAStartup`, the destructor calls `WSACleanup` |
...
| Outside the repo (`%LOCALAPPDATA%\Hamychi\build\Debug`) | 0 / 18 | none |
