...
다만 위 계산은 **외부 경로 MTU가 1500이라는 가정**에 의존한다. PPPoE 회선이나 다른 터널을 경유하는 경로는 이보다 작고, 그 경우 외부 UDP 패킷이 조각화된다. 정확한 값은 실측으로 확정하며, 보수적 고정값을 쓴다면 그 한계를 [`experiments.md`](experiments.md)에 명시한다.
