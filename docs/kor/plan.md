...
| 순서 | 무엇 | 왜 |
|:--:|------|-----|
| 1 | 로컬 기록 파일의 경로와 줄 형식 결정 | [`architecture.md`](architecture.md) 9장에 계약 4개만 있고 형식이 없다. **Phase 4 착수 전에** 그 문서에서 정한다. 코드에서 임의로 정하지 않는다. 정하기 전에는 M-6을 판정할 수 없다 |
| 2 | 구현 Phase 1. Winsock2 래퍼 | 설계 문서가 다 닫혔다. 이제 막는 것이 없다 |
