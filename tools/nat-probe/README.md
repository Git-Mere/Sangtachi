...
엔드포인트 독립인데 필터링 때문에 안 되는 경우를 그대로 놓친다. 이것이 후속 5번이
...
python natprobe.py check-peer --stdin [--ipv4]     # 주소를 표준 입력으로
```

`check-peer` 는 주소가 인터넷에서 도달 가능한 유니캐스트인지만 확인하는 보조 명령이다.
소켓을 열지 않는다. **판정은 종료 코드로만 답한다**(통과 0, 거부 2). 통과하면 아무것도
출력하지 않는다. 거부하면 사유 한 줄을 stderr 에 적지만 **주소는 넣지 않는다** — 판정을
`$?` 나 `$LASTEXITCODE` 로 읽고 stderr 는 사람이 읽는 용도다. 측정 전에 상대가 불러준 주소가
쓸 만한지 보거나, 다른 스크립트에서 검증할 때 쓴다. **판정 구현은 `classify_nat` 과 하나를
공유한다.** 주소는 성공·실패 어느 쪽에도 출력하지 않는다 — 터미널 기록에 남기지 않기 위해서다.
`--stdin` 은 명령줄 인자로 주소를 남기지 않으려 할 때 쓴다.
...
후속 5번은 선택한 안마다 "3회 연속 성공"을 재현 조건으로 요구했다. 한 번의 성공은 판정
근거가 아니다.
...
1. 후속 5번의 선택지 A(NAT 에뮬레이션), B(최소 릴레이 P1 승격), C(제3의 경로 추가) 중
   하나를 지금 고르라는 신호다
...
4. 판정이 나오면 [ADR 0001](../../docs/kor/decisions/0001-직접-연결-대비책-미도입.md)을 갱신한다. 선택지와 M-1/M-4 개정안은 한 묶음이다
...
## 12. 요청하지 않은 인바운드 차단의 원인 좁히기 (선택)

4.3의 `--unsolicited` 가 `blocked` 로 나왔을 때, 그 원인이 **호스트 방화벽인지 NAT인지**
좁히는 절차다. [ADR 0002](../../docs/kor/decisions/0002-리바인딩-복구-미보장.md)의 재검토
조건이 여기를 가리킨다.

**측정은 아직 하지 않았다.** 스크립트 자체는 2026-09-20 에 Windows 에서 처음 실행해
기계 부분(규칙 생성·적용 확인·삭제·정리)까지 확인했다. 실측을 하면 결과를 기록에 남기고
ADR을 갱신한다.
...
관리자 PowerShell, 레포 루트에서:
  .\tools\nat-probe\unsolicited-firewall-test.ps1 -Label us-home
```

**상대 주소를 입력하지 않는다.** 규칙을 포트로만 좁히기 때문이다. 이유는 아래 표를 본다.

**상대도 매 시행마다 같이 돌려야 한다.** 스크립트는 대조와 규칙 시행을 번갈아 `-Trials` 쌍,
즉 **기본 8회** 돌린다. 상대도 8회다. 한 번만 돌리면 첫 시행 뒤로는 상대가 없어 나머지가
전부 무효가 된다.

상대 쪽 명령 (Windows가 아니어도 된다):

```
python tools/nat-probe/natprobe.py punch --label <상대라벨> --port 47000 --unsolicited
```

스크립트가 매 시행 시작 전에 `상대도 지금 시작` 이라고 찍는다. 그때 맞춰 돌린다.

**시행마다 손으로 두 번 입력한다.** `natprobe` 가 상대 엔드포인트를 묻고, 그다음 동시 시작을
맞추는 Enter 를 묻는다. 기본 8회면 입력이 16번이다. 스크립트가 `--peer` 를 넘기지 않기
때문이며, 이는 **상대 주소를 명령줄로 받지 않는다**는 아래 결정의 직접적인 결과다. 상대
엔드포인트는 매 시행 상대 화면에 다시 뜨므로 그것을 쓴다.

스크립트는 [`unsolicited-firewall-test.ps1`](unsolicited-firewall-test.ps1)에 있다.
하는 일은 이렇다.

| 단계 | 내용 |
|------|------|
| 관리자 확인 | 아니면 중단 |
| 단일 실행 보장 | `Global\` named mutex. 동시에 둘이 돌면 같은 포트와 규칙 그룹으로 서로를 방해한다 |
| 남은 규칙 확인 | **지우지 않고 보고만 하고 중단한다.** 무조건 지우면 동시에 도는 다른 시행의 규칙을 없앤다 |
| 대조 / 규칙 시행 | 규칙 없이 한 번, 규칙을 넣고 한 번. **기본 4쌍**(총 8회). `-Trials` 는 **짝수만** 받는다 |
| 순서 뒤집기 | 홀수 쌍은 대조 먼저, 짝수 쌍은 규칙 먼저. 늘 같은 순서면 매핑 워밍업 같은 시간 의존 효과가 방화벽 효과를 흉내 낸다 |
| 라벨 | `<라벨>-p<쌍>-s<순서>-control\|rule`. 결과 파일 이름만 봐도 조건을 안다 |
| 적용 확인 | **`ActiveStore` 에서** `Enabled`/`Action`/`Direction` 을 본다. 만든 객체의 `Enabled` 만 보면 GPO 병합으로 적용되지 않은 규칙을 활성으로 오인한다 |
| 정리 | `try/finally` 로 **그 규칙 객체만** 지우고, **`ActiveStore` 에서 사라질 때까지** 최대 5초 기다린다. 적용 중인 규칙이 잠깐 남아 다음 대조 시행을 오염시킬 수 있다 |

`$ErrorActionPreference = 'Stop'` 을 쓰고 방화벽 조회는 `Get-TestRuleSafe` 로 감싼다.
**조회 실패를 "규칙 없음" 으로 읽으면** 남은 규칙을 못 보고 측정하거나 정리가 끝났다고
잘못 보고한다.
...
| 이유 | 설명 |
|------|------|
| 정확성 | 상대 주소로 좁히면 요청하지 않은 패킷의 출발지 IP 가 그 주소와 같다고 **가정**하게 된다. 주소 의존 NAT 이면 규칙이 아예 걸리지 않고, 그 결과를 "방화벽이 원인이 아니다" 로 잘못 읽는다 |
| 노출 | 주소를 입력받아 다루는 동안 PowerShell 기록, 프로세스 목록, 트랜스크립트로 샌다. 입구를 없애는 편이 확실하다 |

**대가는 규칙이 넓어지는 것이다.** UDP 한 포트에 대해 모든 출발지로 열린다. 정상 종료하면
매 시행 끝에 `ActiveStore` 확인까지 하고 지운다.

> ⚠️ **비정상 종료하면 규칙이 영구히 남는다.** `New-NetFirewallRule` 은 기본으로
> `PersistentStore` 에 규칙을 만든다. 프로세스를 강제 종료하거나 창을 닫거나 재부팅하면
> `finally` 가 돌지 않아 **포트가 열린 채로 남는다.** 스크립트가 시작할 때 이 경고와 삭제
> 명령을 찍는다. 그런 일이 있었으면 직접 확인한다.
>
> ```powershell
...
**쌍으로 비교한다. 한 번의 변화는 근거가 아니다.** NAT 상태와 시각도 시행마다 달라지므로
대조와 규칙 시행을 번갈아 돌려 일관된 차이가 있는지 본다.

**두 순서가 모두 시험됐을 때만 인과를 말한다.** 아래 판정은 `-Trials` 로 설정한 모든 쌍에
적용되며, `control` 먼저와 `rule` 먼저가 둘 다 있어야 한다. 한 순서만 돌았다면 결과가
무엇이든 순서 효과와 구분되지 않으므로 판정 불가다.

| 관측 | 해석 | 조치 |
|------|------|------|
| **모든 쌍**에서 `control`·`rule` 둘 다 `blocked` | **기본 인바운드 정책은 원인이 아니다.** 방화벽 전체를 배제하지는 못한다 — 명시적 차단 규칙이 허용 규칙보다 우선하고, 제3자 보안 제품의 필터링도 남는다. NAT 쪽으로 기울지만 확정은 아니다 | ADR 0002 유지 |
| **모든 쌍**에서 `control` 은 `blocked`, `rule` 은 `allowed` | **Windows 방화벽이 원인이라는 것과 일치한다.** 단독으로 증명은 아니지만, 순서를 뒤집어도 같은 차이가 나오면 강한 증거다 | ADR 0002 개정. 옵션 a 재검토, 관리자 권한 요구를 `spec.md`와 후속 3번에 반영 |
| 결과가 쌍마다 다르다 | 판정 불가. NAT 상태, 시각, 상대 쪽 조건이 흔들린다 | 시행 수를 늘리거나 조건을 고정하고 다시 |

확정하려면 유효 WFP 규칙 확인이나 양쪽 패킷 캡처가 필요하다. 이 절차는 거기까지 가지 않는다.
