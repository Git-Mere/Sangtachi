...
제어 평면이 죽어도 **이미 수립된 P2P 터널은 계속 동작**해야 한다. 제어 평면은 세션 수립에만 필요하다.

**텔레메트리 수집은 제어 평면이 맡지 않는다.** 별개 서비스이고 배포 단위가 다르다. 두 서비스는 같은 EC2 인스턴스에 서로 다른 프로세스로 뜨지만 의존은 한 방향도 없다. 경계 계약은 3.4에 있다.
...
초기 안은 `[main]` / `[net_rx]` / `[tun_rx]` / `[timer]` 4스레드였다. 두 가지 이유로 폐기했다. 첫째, 네 스레드가 세션 상태와 송신 `sequence`를 동기화 없이 변경한다. 둘째, 텔레메트리 업로드가 타이머 스레드를 공유해 **업로드 대상 장애가** keepalive를 멈춘다. 후자는 NFR-3을 설계 단계에서 위반한다.
...
**어댑터가 없는 빌드(Phase 4~5)에서 8.4를 통과한 `DATA`가 갈 곳도 여기서 정한다.** 그 구간에는 주입할 어댑터가 없으므로 [`protocol.md`](protocol.md) 8.4의 "Wintun 주입 직전" 이 가리키는 다음 단계가 비어 있다.
...
**훅을 시험 전용 경로로 따로 짜지 않는다.** 8.1~8.4를 통과한 뒤의 같은 지점에 걸어야 그 시험이 실제 수신 경로를 검증한다. 7장 출발지 위생 시험이 수신 **진입점**을 하나로 못박은 것과 같은 이유이고, 여기는 그 대칭인 **출구**다. `drop_no_sink`는 제품 빌드에서 0이 아니면 설계가 어긋난 것이므로 그 자체가 진단이다.
...
`drain_console`은 `[console]`이 큐에 넣은 명령 줄을 비운다. 이벤트가 자동 리셋이라 별도 리셋 호출이 필요 없다. Phase 6 이후에는 이 핸들도 스레드도 사라진다. **`quit` 명령은 종료 이벤트를 신호한다.** `shutdown()`을 직접 부르지 않는다. 그러면 `[telemetry]`와 `[control]`이 같은 신호로 함께 깨어나고, 다음 바퀴의 대기가 종료 핸들에서 반환해 3.2.7의 순서를 그대로 탄다. 3.2.7 (1)이 "아직 안 됐으면"이라고 적은 것이 이 경로다.

**콘솔이 만든 송신 잔량은 `busy`에 포함한다.** `send <n>`(Phase 4~5)과 `raw`(3.5)는 한 바퀴에 몰아넣지 않고 `MAX_DRAIN`과 같은 방식으로 나눠 보내므로, 잔량이 남은 바퀴는 `busy = true`여야 다음 대기가 타임아웃 0으로 즉시 돌아온다. 그렇지 않으면 남은 송신이 타이머 간격만큼씩 늘어진다. `drain_control`은 `[control]`이 응답 큐에 넣은 결과를 비우고 세션 상태에 반영한다(3.2.8). 두 큐는 작고 요청이 사람 속도이므로 예산을 두지 않는다.
...
**어댑터 읽기는 `adapter_rx`로 센다.** `WintunReceivePacket`이 패킷 하나를 돌려줄 때마다 오른다. Phase 6 검증이 "어댑터 read 카운터" 로 읽는 값이 이것이다([`roadmap.md`](roadmap.md)). 주입 쪽은 성공을 세지 않는다. 실패만 `drop_inject_error`로 센다.

**이 절의 카운터는 [`protocol.md`](protocol.md)의 카운터와 계층이 다르다.** `drop_oversize_datagram`, `drop_inject_error`, `drop_no_sink`, `adapter_rx`는 소켓과 어댑터 API가 낸 결과를 세는 것이라 이 문서가 정의한다. protocol.md 7장과 8장의 `drop_*`는 와이어 분류와 검증 파이프라인의 카운터다. 목록을 만들 때 두 출처를 섞지 않는다.
...
- 타이머 집합이 작다. [`protocol.md`](protocol.md) 11장의 표가 한 자릿수이고 최소 범위에서 세션은 하나다. 같은 장의 "만료 값" 표(폐기 목록, nonce, probe, `pending_pings`)는 세션 타이머가 아니라 **항목마다 붙는 수명**이라 그 항목을 훑을 때 함께 본다. 매 바퀴 선형 주사로 가장 이른 마감을 구한다. 타이머 휠은 필요 없다.
...
세션 상태 머신, `session_epoch`, 송신 `sequence`, 재생 방지 비트맵, `peer_endpoint`, 마지막 수신 시각, 후보 목록, `pending_pings`, 모든 폐기 카운터, **로컬 기록 파일의 핸들과 쓰기**(9장), 그리고 `room_id`·`peer_id`·`peer_token`·상대 피어 정보 같은 **제어 평면에서 받은 값.** `[control]`은 그 값을 응답 큐로 넘길 뿐 자기 상태로 들고 있지 않다. 예외는 3.2.8이 적은 해석된 서버 주소 하나다.
...
- **콘솔 명령 큐는 16 줄이고 같은 정책을 쓴다.** 가득 차면 새 줄을 버리고 `console_queue_dropped` 를 올린다. **이 카운터만 3.2.5 의 단독 소유 규칙에서 예외다.** 버리는 쪽이 생산자인 `[console]` 이라 그 스레드가 올린다. 원자 변수 하나이고 `[loop]` 는 읽기만 한다(9장의 `counter` 이벤트). 다른 폐기 카운터는 전부 `[loop]` 가 올린다. **두 큐의 카운터를 같은 이름으로 쓰지 않는다.** 이름이 없으면 Phase 1 검증이 무엇을 보는지 정할 수 없다. 생산자가 `[console]`, 소비자가 `[loop]` 하나씩이라 여기도 락이 필요 없다. 사람이 치는 명령이 16줄이나 밀리는 상황은 `[loop]`가 이미 막혔다는 뜻이고, 그때 큐를 키워도 처리되지 않는다.
...
- **데이터 평면에 락이 하나도 없다.**
- `[telemetry]`가 큐를 비워 텔레메트리 서비스에 올린다. 업로드가 블록되든 실패하든 `[loop]`와 무관하다.
- 텔레메트리는 **텔레메트리 서비스로 가는 자체 TCP 소켓**을 쓴다. 제어 평면 소켓과 공유하지 않는다. [`protocol.md`](protocol.md) 6장의 단일 소켓 제약은 **UDP 데이터 평면에만** 적용된다.

Phase 5의 "서버 중단 10분" 시험(NFR-3)은 이 격리 없이는 통과할 수 없다. 4스레드 안에서는 응답하지 않는 **텔레메트리 서비스**로의 TCP 업로드가 타이머 스레드를 수십 초 막고, 그 사이 keepalive가 나가지 않아 NAT 매핑이 만료된다. 원격 서비스 장애가 터널을 끊는다. **서비스를 둘로 나눠도 이 격리는 여전히 필요하다.** 분리는 텔레메트리 장애가 제어 평면을 막지 않게 할 뿐이고, 클라이언트 안에서 업로드가 `[loop]`를 막는 문제는 스레드 격리만이 막는다.
...
- `[loop]`의 `shutdown()`은 순서대로 (1) 종료 이벤트 신호(아직 안 됐으면), (2) [`protocol.md`](protocol.md) 5.6이 정한 대상 세션마다 `CLOSE` 1회 송신 (대상과 reason 값은 그 절이 정한다. 여기에 조건을 다시 적지 않는다), (3) **아직 끝나지 않은 세션이 `CONNECTED`에 도달했었다면 로컬 기록 파일에 종료 줄을 쓴다**(9장. 이미 종료 상태라 그 줄을 쓴 세션은 다시 쓰지 않는다), (4) **카운터 전량을 `counter` 이벤트로 낸다**(9장), (5) 어댑터 세션 종료, (6) 어댑터/주소/라우트 정리, (7) **정리 완료 이벤트 신호**, (8) `[telemetry]`와 `[control]` join을 수행한다. `[control]`은 Phase 3 이후에만 있으므로 그 전에는 (8)이 `[telemetry]`만이다.
- **종료 상태에 도달하면 프로세스도 끝난다.** 세션이 `FAILED`나 `CLOSED`가 되면([`protocol.md`](protocol.md) 9.6, 5.6) `[loop]`는 그 세션의 기록 줄과 카운터를 남긴 뒤 스스로 `shutdown()`으로 간다. 최소 범위의 세션은 하나이므로 그 세션이 끝나면 프로세스가 할 일이 없다. 사용자의 `quit`을 기다리지 않는다. **자동 재시도를 정하면 이 줄을 다시 본다**(`protocol.md` 10.4의 미정 항목). 재시도가 생기면 `FAILED`가 곧 종료가 아니게 된다.
...
- join 상한은 두 스레드 합쳐 2초다. 넘기면 남은 레코드와 진행 중인 제어 요청을 포기하고 join을 건너뛴 채 `_exit`로 즉시 끝낸다. 정리가 (7)에서 이미 끝났으므로 안전하다. **상한을 실제로 강제하는 것은 소켓 타임아웃이 아니라 이 `_exit`다.** 소켓 타임아웃은 흔한 경우를 깔끔하게 끝내줄 뿐이고, 어떤 경우에도 종료가 텔레메트리 서비스 가용성에 인질 잡히지 않게 하는 것은 `_exit`다.
...
| STUN 서버 | `--stun <이름 또는 IPv4>:<포트>` 반복 | 아니오 | 주면 아래 기본 목록을 **통째로 대체**한다. **목록이 한 개면 기동 시 `WARN` 한 줄을 남긴다.** 그 목록으로는 서로 다른 두 서버의 응답을 얻을 수 없어 아래 "서버 선택" 대로 `STUN_DISCOVERY_FAILED` 로 끝난다. 경고를 기동 시점에 내는 이유는 5초를 기다린 뒤 실패를 보는 것보다 입력이 틀렸다는 것을 먼저 알리는 편이 낫기 때문이다 |
...
**서버 선택.** 목록의 앞 두 서버에 같은 소켓으로 **동시에** 질의한다. 트랜잭션 ID 가 응답을 가른다([`protocol.md`](protocol.md) 13장). **STUN 마감은 서버별이다**([`protocol.md`](protocol.md) 11장). 한 서버가 자기 마감에 걸리면 목록의 다음 서버로 바꿔 다시 질의하고, 그 서버의 재시도 일정과 마감은 새로 시작한다. 목록을 다 써도 **서로 다른 두 서버의 응답**을 얻지 못하면 `STUN_DISCOVERY_FAILED` 다. **응답이 하나뿐이어도 실패다.** M-2 와 "지원 네트워크 조건" 판정에 둘이 필요하고, 하나로 진행하면 목적지 의존 매핑을 구분하지 못한 채 펀치에 들어가 그 실패가 NAT 실패로 기록된다. 단계 전체의 상한은 그 문서가 계산한다.

**STUN 서버 이름은 기동 시 한 번 해석한다.** 기본 목록이 전부 DNS 이름이므로 해석이 필요하고, 그 호출(`getaddrinfo`)은 동기라 `[loop]` 안에 두지 않는다. **`[loop]` 시작 전에 목록 전체를 해석한다.** Phase 1~2 에는 `[control]` 스레드가 없으므로(3.2.1) 그 자리를 빌릴 수도 없다. 결과가 여럿이면 **첫 IPv4 주소 하나**를 쓴다([`control_plane.md`](control_plane.md) 3.2 와 같은 규칙이다). `AAAA` 는 쓰지 않는다. **해석에 실패한 항목은 목록에서 빼고 `WARN` 한 줄을 남긴다.** 남은 목록이 두 개 미만이면 위 규칙대로 `STUN_DISCOVERY_FAILED` 다. 제어 서버 주소와 달리 여기서는 해석 실패가 기동 실패가 아니다. 목록이 여럿이라 하나가 죽어도 진행할 수 있다.
...
**Phase 1~2 의 시험용 인자 둘.** 위 표는 제품 경로의 입력이다. Phase 1 은 제어 평면도 STUN 도 없이 "알려진 두 엔드포인트" 로 송수신을 확인하므로([`roadmap.md`](roadmap.md) Phase 1) 상대 주소를 받을 자리가 필요하다.
...
**수신 측은 `rx.raw` 로그 한 줄을 낸다.** 필드는 `from`(출발지 `IPv4:port`), `len`, `sha256`(수신 바이트열의 16진 해시 앞 16자)이다. 바이트열 자체를 로그에 싣지 않는다. **송신 측도 같은 줄을 낸다.** 두 줄의 `len` 과 `sha256` 이 같으면 Phase 1 의 "바이트 단위로 일치" 가 성립한다. 눈으로 바이트를 비교하는 절차를 두지 않는 이유는 그것이 사람마다 다른 판정이 되기 때문이다.
...
**인자 오류의 거동.** 알 수 없는 인자, 역할 누락, 형식 위반(`--room` 이 6자가 아님, `--rejoin` 이 `<peer_id>:<peer_token>` 꼴이 아님, 포트가 1~65535 밖)은 전부 **`ERROR` 한 줄을 남기고 종료 코드 2로 기동 실패**다. 추측해서 고쳐 받지 않는다. `peer_id` 는 **10진수**로 적는다. 같은 인자를 두 번 주면 `--stun` 은 목록에 쌓이고 나머지는 마지막 값이 이긴다.
...
**두 연산은 실패해도 클라이언트가 재시도하지 않는다.** 다만 둘은 주기가 다르다. `report_telemetry` 는 주기 업로드라 실패하면 다음 주기로 넘어간다. **`report_connection` 은 연결 시도 한 건마다 한 번뿐이라 넘어갈 다음 주기가 없다.** 그 시도의 업로드는 그것으로 끝이고, 같은 내용이 로컬 기록 파일에 이미 남아 있다(9장). 근거는 [`spec.md`](spec.md) NFR-3과 9장 로컬 기록 계약이다. 판정에 쓰이는 것은 로컬 기록이지 이 업로드가 아니다.

**실패를 삼키지는 않는다.** 업로드가 실패하면 `[telemetry]` 가 카운터 `telemetry_upload_failed` 를 올리고 로그에 한 줄 남긴다. 큐가 가득 차 버리는 것(`telemetry_queue_dropped`, 3.2.6)과 **다른 카운터다.** 앞의 것은 원격이 받지 못한 것이고 뒤의 것은 보내기 전에 버린 것이라, 둘을 한 이름으로 세면 텔레메트리 서비스 장애와 클라이언트 과부하를 구분할 수 없다. **이 실패는 로컬 기록 파일에 쓰지 않는다.** 9장 로컬 기록은 연결 시도 한 건마다 한 줄이라는 계약이고, 업로드 실패는 연결 시도가 아니다.
...
| `CONTROL_PLANE_EXCHANGE_FAILED` | 후보 교환 | `get_peers` 폴링 마감 초과, 제어 평면 오류, `peer_id` 충돌. 조건 목록은 [`protocol.md`](protocol.md) 9.6이 갖는다 |
...
클라이언트는 로컬 링 버퍼에 먼저 쌓고 전용 스레드가 주기적으로 **텔레메트리 서비스**에 보낸다. 업로드는 데이터 평면과 완전히 분리되어 있어 텔레메트리 서비스 장애가 터널에 영향을 주지 않는다. 격리 구조는 3.2.6에 있다.
...
- **종료 줄**: **`CONNECTED` 에 도달한 시도에만 쓴다.** 종료 코드와 마지막 RTT 표본, 없으면 빈 값. 종료 코드는 `CLOSED` 또는 [`spec.md`](spec.md) FR-13 의 실패 코드 하나다. `TUNNEL_DROPPED` 가 흔한 경우이고, **재협상으로 `CONNECTED` 이전으로 돌아간 뒤 펀치 마감에 걸리면 `HOLE_PUNCH_TIMEOUT` 이나 `PEER_HANDSHAKE_FAILED` 가 종료 줄에 온다**([`protocol.md`](protocol.md) 9.5, 9.6). 코드 집합을 둘로 못박으면 그 경우에 쓸 값이 없다
...
응답기와 같은 CMake 옵션이고([`roadmap.md`](roadmap.md) Phase 4), 켜진 빌드는 기동 시
`WARN` 한 줄을 남긴다.
