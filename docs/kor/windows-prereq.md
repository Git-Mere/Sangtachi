...
| 표시 | 뜻 |
|------|-----|
| **실측** | 2026-09-21 에 Windows 11 24H2 한 대에서 확인 명령을 돌렸고 통과 조건까지 그 출력으로 판정된다 |
| **부분 실측** | 명령은 돌렸으나 **통과 조건이 아직 판정되지 않는다.** 무엇이 빠졌는지 적는다 |
| **미검증** | 돌려 보지 않았다. 사유와 무엇이 있으면 검증되는지 적는다 |

현재 실측 4절, 부분 실측 4절, 미검증 5절이다.

**조회 실패와 "없음" 을 구분한다.** 진행 여부를 가르는 조회(3절, 8절)는 `ObjectNotFound` 만
0 으로 돌리고 나머지 오류는 올린다. 2절과 7절은 판정을 스크립트로 옮겼고, 그 스크립트는 같은
구분을 `unknown` 판정으로 낸다. **`unknown` 은 통과가 아니다.** 권한 부족을 "없음" 으로 읽으면 잔존물을 못 본 채
진행한다. **참고용 조회**(13절의 정책 값과 `Zone.Identifier`)는 `-ErrorAction
SilentlyContinue` 를 쓰고, 없는 것이 정상이라는 뜻을 그 자리에 적는다.
...
"식별되지 않은 네트워크"로 분류되고, 그러면 **Public 프로필**이 붙는다. Public 에서는
ICMP 에코 요청과 TCP 25565 인바운드가 막힌다. 터널 UDP 중 **우리가 먼저 보낸 흐름에 대한 응답**은
상태 기반 처리로 통과한다([`plan.md`](plan.md) 5.8 실측). **요청하지 않은 인바운드는 그렇지
않다.** 같은 실측에서 6건 전부 `blocked` 였다. 그쪽이 필요하면 인바운드 허용 규칙이 있어야
한다.
...
판정을 [`tools/winprereq/Test-FirewallPolicy.ps1`](../../tools/winprereq/Test-FirewallPolicy.ps1)
로 옮겼다. 케이스 225건은 스크립트 안에 있고 `-SelfTest` 로 돈다.
...
**영어 낱말로 줄을 거르지 않는다.** `netsh` 의 **레이블**은 표시 언어에 따라 번역되므로
`Select-String 'Profile Settings|State'` 같은 필터는 한국어 표시 Windows 에서 아무것도
못 잡는다. **값인 `BlockInbound` 는 키워드라 번역되지 않는다.** 이것이 값으로만 판정하는
이유다.
...
> `DefaultInboundAction -eq 'Block'` 으로 검사를 쓰면 **정상 기기에서 거짓이 나온다.**
...
다른 쪽이 안 된다. 꺼진 규칙은 덮음으로 세지 않는다.
스크립트의 `endpoint-coverage` 줄이 둘 다 덮였는지 판정한다. 넓은 규칙이나 엉뚱한 어댑터에 묶인
규칙은 덮은 것으로 치지 않는다. **규칙이 제대로 좁혀졌는지는 스크립트의
`-InterfaceAlias` 모드가 판정한다.**
...
**무조건 지우지 않는다.** 이름이 겹치는 남의 어댑터를 지울 수 있다. 우리가 만든 것인지는
`InterfaceDescription` 이 Wintun 인지로 함께 확인한다.
...
**`-ErrorAction SilentlyContinue` 로 "없음" 을 판정하지 않는다.** 권한 부족이나 서비스 이상도
같은 무출력이 되어 **잔존물을 못 본 채 깨끗하다고 보고한다.** 8절의 `Get-RouteSafe` 와 같은
구분이며, 셋을 하나의 래퍼로 묶은 것이다.
...
1. 서버 프로세스가 그 주소에 바인드했는가 — 서버 PC 에서 아래를 돌린다
...
판정이다. **`netstat -ano | findstr 25565` 는 쓰지
않는다** — 원격 포트가 25565 인 줄도 걸리고 리스닝인지 아닌지 구분되지 않는다.
2. 서버 PC 방화벽이 25565 인바운드를 막는가 — 2절
3. 터널이 그 패킷을 옮기는가 — 터널 계층 문제이지 이 절이 아니다

## 6. 제어 평면 EC2 (blocker 16)
...
**위 조회에서 `[::]:8000` 만 보이는 경우를 통과로 단정하지 않는다.** Linux 기본값
(`net.ipv6.bindv6only=0`)에서는 이중 스택 소켓이 IPv4 도 받지만 **그 sysctl 은 바꿀 수
있다.** 그래서 IPv4 전용 조회를 판정으로 쓰고, 최종 확인은 아래 외부 접속으로 한다.
...
**검증: 부분 실측.** 판정을 [`tools/winprereq/Test-SubnetOverlap.ps1`](../../tools/winprereq/Test-SubnetOverlap.ps1)
로 옮겼고 이 기기에서 돌렸다. 케이스 85건은 스크립트 안에 있고 `-SelfTest` 로 돈다.
...
**분류: 클라이언트.** 차단 조건이면 대체 대역을 고르거나 중단한다.

**Windows 는 최장 접두사 일치로 라우트를 고른다.** 우리가 만들 것은 `/24` on-link 라우트다.
겹치는 라우트를 **접두사 길이로** 세 부류로 나눈다. **이 표가 판정 기준의 출처다.**
...
동작하지 않는 것보다 낫지 않다.** 대체 대역 메커니즘이 있으므로 비용도 작다. 그 대역이
...
**거짓 통과보다 거짓 차단이 낫다.** 3절의 "이름이 겹치는 남의 어댑터를 지우지 않는다" 와 같은
규칙이다.
...
**막혔을 때.** 대체 대역을 쓴다. 대역이 바뀌면 양쪽이 같은 값을 써야 하므로 제어 평면이
알려주는 값이어야 한다. **클라이언트가 혼자 정하면 두 쪽이 다른 대역을 쓴다.**

## 8. on-link 라우트 중복 생성 (warn)
...
**`-ErrorAction SilentlyContinue` 로 때우지 않는다.** 그러면 "없음" 과 "권한 부족이나 서비스
이상으로 못 봤음" 이 같은 결과가 된다. 없다고 잘못 읽고 만들면 중복 생성으로 실패하고,
그때 원인은 이미 지워져 있다. **없음만 0 으로 돌리고 나머지는 올린다.**
...
**형식으로 잡지 않는 이유는 `unsolicited-firewall-test.ps1` 의 `Get-TestRuleSafe` 와 같다.**
같은 "일치 항목 없음" 이 예외 형식을 바꿔 가며 오는 사례를 그쪽에서 먼저 겪었다. 3절의
잔존물 조회도 같은 구분이 필요하다. **오류를 삼키지 않는다.** 중복이라
무시해도 되는 오류와 권한 부족은 다르게 처리해야 한다.

## 9. 주소 tentative 상태 (warn)
...
**시간으로 기다리지 않는다.** `Start-Sleep 2` 같은 고정 대기는 느린 기기에서 깨지고
빠른 기기에서 낭비다. 상태를 폴링한다.
...
**대가.** 공인 IPv4 주소는 **인스턴스에 붙어 있어도** 시간당 요금이 붙는다. 붙어 있지 않을
때만 과금되던 시절의 규칙이 아니다. 시연이 끝나면 인스턴스를 지우고 Elastic IP 도 해제한다.
**요금 체계는 바뀌므로 쓰기 전에 현재 요금 페이지를 확인한다.**

## 11. Minecraft LAN 탐색은 안 된다 (warn)

**검증: 미검증 (문헌 근거).** 실제로 시도해 보지 않았다. Phase 8 에서 확인한다.

Minecraft **Java Edition** 의 "LAN 에 공개" 는 **멀티캐스트**로 광고한다. 널리 인용되는 값은
...
## 13. SmartScreen 과 Defender (warn)
...
Mark of the Web 이 붙어 더 엄격해진다. **우리 빌드 산출물이 여기에 걸릴 위험이 있다. 관측한 것이 아니라 예상이다** — `client.exe`
가 아직 없다. 로컬에서 빌드한 파일에는 보통 MOTW 가 붙지 않으므로, 실제로 경고가 뜨는지는
**그 바이너리를 시연 PC 에 올린 방법에 달려 있다.** Phase 8 에서 그 파일로 확인한다.
...
**보장은 아니다.** NTFS 로 포맷된 매체는 대체 데이터 스트림을 그대로 나를 수 있고,
내려받은 압축 파일을 Windows 기본 탐색기로 풀면 MOTW 가 따라붙을 수 있다. **압축 도구와
Windows 판에 따라 다르다.** 어느 경로로 옮겼든 위의 `Zone.Identifier` 조회로 확인한다.
...
**"한 번만 막힌다" 는 그 파일에 한해서다.** 허용한 뒤에는 **같은 시연 PC 의 같은 파일**이
다시 묻지 않는다. 바이너리를 다시 빌드하거나, 다른 경로로 옮기거나, 다른 계정으로 실행하거나,
새로 내려받으면 **다시 묻는다.** 시연 직전에 바이너리를 바꿨다면 그 파일로 한 번 더 실행해
둔다.
