...
**All code that touches tunnel state runs on one thread only.** There are four threads,
`[loop]`, `[telemetry]`, `[console]` and `[control]`, and `[loop]` is the process main thread.
The other three exist only in the phases that need them.

The thread set, the wait set, the order inside one loop iteration, timers, state ownership, the
shutdown order and the `[control]` thread are held by [`concurrency.md`](concurrency.md).
...
- **The whole list is resolved before `[loop]` starts.** Phase 1~2 has no `[control]` thread
  ([`concurrency.md`](concurrency.md) chapter 1 Threads), so that thread cannot be borrowed for
  it either
...
| Shutdown | Console command `quit` | Signals the shutdown event. `concurrency.md` chapter 3 One Loop Iteration owns that path |
...
dropping because the queue is full (`telemetry_queue_dropped`,
...
telemetry service outage does not affect the tunnel. The isolation structure is in
[`concurrency.md`](concurrency.md) chapter 6 Telemetry Isolation.
...
- In the process shutdown procedure ([`concurrency.md`](concurrency.md) chapter 7 Shutdown),
  all of them
...
|   |   +-- concurrency.md       confirmed concurrency model
