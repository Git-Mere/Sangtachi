...
| NFR-5 | Non-basic third-party dependencies require instructor approval **before** integration, and the scope each library provides is documented. Wintun was approved on 2026-09-14. |
...
| C-4 | The work must be completable within one semester. Course milestones fall around weeks 5, 10, and 15. |
| C-5 | On scope overrun, drop from the lowest priority upward. P0: UDP + STUN + endpoint exchange + hole punching + base tunnel header (Phases 1-4) / P1: DATA transport + protocol robustness + session state machine (Phase 5) / P2: Wintun + virtual IP + Minecraft (Phases 6-8) / P3: telemetry + diagnostics + analysis (Phase 9) / P4: encryption + relay + GUI + extra platforms (stretch). This must stay identical to the priority block in [`roadmap.md`](roadmap.md). |
...
| M-2 | Both machines discover a usable public endpoint through the custom STUN client. | Each machine logs its local and public endpoint, and the public IP matches an external lookup service. Whether the discovered IP:Port is actually receivable is confirmed in M-4/M-5 |
...
| M-5 | Arbitrary packets or application data are transferred over the direct tunnel. | Bidirectional message exchange succeeds and the received content matches what was sent |
| M-6 | Connection status and basic performance data are recorded. | The control plane database holds connection result records and RTT values |
...
| A-2 | Successful and failed NAT traversal cases are compared. | Every failure records a per-stage code and NAT environment information, aggregated by observed mapping behavior. NAT type is never asserted. **Even if no failure occurs in real environments**, a controlled failure scenario (blocking UDP at the firewall, injecting a nonexistent endpoint) is included as a required case |
